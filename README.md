...
- Content (bio/experience) is sourced from `cv/GiacomoCastiglioniCV.pdf`.
- When editing `css/resume.css` or `js/resume.min.js`, bump the `?v=N` cache-busting query in `index.html`.
- `js/resume.min.js` is minified by hand — keep it in sync with `js/resume.js`.
